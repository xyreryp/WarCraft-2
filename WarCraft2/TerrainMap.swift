...
//    }
        ...
        for yOff in 0..<2 {
            for xOff in 0..<2 {
                if(DRendered) {
                    ...
                    var index: Int = 0
                    var xPos: Int = xindex + xOff
                    var yPos: Int = yindex + yOff
                    if((0 < xPos)&&(0 < yPos)) {
    ...
    func CalculateTileTypeAndIndex(_ x: Int, _ y: Int, _ type: inout ETileType, _ index: inout Int) {
        var UL = DTerrainMap[y][x]
        var UR = DTerrainMap[y][x+1]
        var LL = DTerrainMap[y+1][x]
        var LR = DTerrainMap[y+1][x+1]
        
        
        var Temp1 = ((DPartials[y][x] & 0x8)>>3)
        var Temp2 = ((DPartials[y][x+1] & 0x4)>>1)
        var Temp3 = ((DPartials[y+1][x] & 0x2)<<1)
        var Temp4 = ((DPartials[y+1][x+1] & 0x1)<<3)
        var TypeIndex: Int = Temp1 | Temp2 | Temp3 | Temp4
    ...
    func RenderTerrain() {
        ...
        for YPos in 0..<DMap.count {
            if((0 == YPos)||(DMap.count - 1 == YPos)){
                for XPos in 0..<DTerrainMap[0].count + 1 {
