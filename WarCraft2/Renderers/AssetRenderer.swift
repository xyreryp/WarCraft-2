...
class CAssetRenderer {
    ...
    func TestDrawAssets(surface: SKScene, tileset _: [CGraphicTileset]) {
        for AssetIterator in DPlayerMap.DAssets {
            let DX = AssetIterator.PositionX() * 16 + 8
            let DY = -AssetIterator.PositionY() * 16 - 8
            //            let DX = AssetIterator.PositionX() + (AssetIterator.Size() - 1) * CPosition.HalfTileWidth() - DTilesets[AssetIterator.DType.DType.rawValue].TileHalfWidth()
            //            let DY = AssetIterator.PositionY() + (AssetIterator.Size() - 1) * CPosition.HalfTileHeight() - DTilesets[AssetIterator.DType.DType.rawValue].TileHalfHeight()
            print("Drawing asset \(AssetIterator.DType.DName)")
            print("Position X = \(AssetIterator.PositionX()), DX = \(DX)")
            print("Position Y = \(AssetIterator.PositionY()), DY = \(DY)")
            switch AssetIterator.DType.DName {

            case "Peasant":
                DTilesets[0].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 0)
                break
            case "Footman":
                DTilesets[2].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 0)
                break
            case "Archer":
                DTilesets[3].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 0) // Archer
                break
            case "Ranger":
                DTilesets[4].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 0) // Ranger
                break
            case "GoldMine":
                DTilesets[5].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 1) // Goldmine
                break
            case "TownHall":
                DTilesets[6].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 3) // Townhall
                break
            case "Keep":
                DTilesets[7].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 1) // keep
                break
            case "Castle":
                DTilesets[8].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 1) // castle
                break
            case "Farm":
                DTilesets[9].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 3) // Farm
                break
            case "Barracks":
                DTilesets[10].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 3) // Barracks
                break
            case "LumberMill":
                DTilesets[11].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 3) // lumberMill
                break
            case "Blacksmith":
                DTilesets[12].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 2) // Blacksmith
                break
            case "ScoutTower":
                DTilesets[13].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 2) // ScoutTower
                break
            case "GuardTower":
                DTilesets[14].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 0) // GuardTower
                break
            case "CannonTower":
                DTilesets[15].DrawTile(skscene: surface, xpos: DX, ypos: DY, tileindex: 0) // Cannon Tower
                break
            default:
                break
            }
    ...
    //    func DrawAssets(surface: SKScene, /* typesurface _: CGraphicSurface, */ rect: SRectangle) {
