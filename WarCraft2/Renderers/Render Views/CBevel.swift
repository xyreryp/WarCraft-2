...
class CBevel: NSObject {
    ...
    func DrawBevel(context:CGraphicResourceContext, xpos:Int, ypos:Int, width:Int, height:Int) {
        let TopY = ypos - DWidth
        let BottomY = ypos + height
        let LeftX = xpos - DWidth
        let RightX = xpos + width
        
        DTileset.DrawTile(context: context, xpos: LeftX, ypos: TopY, titleindex: DCornerIndices[0])
        DTileset.DrawTile(context: context, xpos: RightX, ypos: TopY, titleindex: DCornerIndices[1])
        DTileset.DrawTile(context: context, xpos: LeftX, ypos: BottomY, titleindex: DCornerIndices[2])
        DTileset.DrawTile(context: context, xpos: LeftX, ypos: BottomY, titleindex: DCornerIndices[3])
        ...
        while XOff < width {
            ...
            DTileset.DrawTile(context: context, xpos: xpos + XOff, ypos: TopY, titleindex: DTopIndices[Index])
            DTileset.DrawTile(context: context, xpos: xpos + XOff, ypos: BottomY, titleindex: DBottomIndices[Index])
        ...
        while YOff < height {
            ...
            DTileset.DrawTile(context: context, xpos: LeftX, ypos: ypos + YOff, titleindex: DLeftIndices[Index])
            DTileset.DrawTile(context: context, xpos: RightX, ypos: ypos + YOff, titleindex: DRightIndices[Index])
